...
type Props = {
  ...
  items: any[]; // Array of submenu items
...
function FirstMenu({
  ...
  return (
    <div
      className={`absolute top-13 mt-2 w-100 text-right right-41 rounded-lg z-20 border-b border-white transition-all cursor-pointer duration-300 ease-in-out ${
        isOpen ? "block" : "hidden"
      }`}
    >
          ...
          {item.submenu ? (
            <>
              ...
              <button
                ...
                className={`group w-full text-right px-4 py-2 bg-quaternary-bg cursor-pointer border-b border-white flex flex-col items-end focus:outline-none ${
                  activeSubMenu === item.label ? "text-primary" : "text-white"
                } hover:text-primary`}
              >
            ...
            <Link
              ...
              className={`group w-full text-right px-4 py-2 focus:outline-none ${
                activeSubTab === item.label ? "text-primary" : "text-white"
              } hover:text-primary`}
            >
              <span>{item.label}</span>
            </Link>
