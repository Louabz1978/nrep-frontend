...
function ApexScatterCard() {
  ...
  const getPropertyFields = (propertyType: string) => {
    const fieldMap: Record<string, { totalPrice: string; soldCount: string }> = {
      apartment: { totalPrice: "apartments_total_price", soldCount: "apartments_sold" },
      ...
      building: { totalPrice: "buildings_total_price", soldCount: "buildings_sold" },
    };
    return fieldMap[propertyType] || { totalPrice: "apartments_total_price", soldCount: "apartments_sold" };
  };

  const brokersOnly = useMemo(() => {
    if (!brokerHistory) return [];
    const dataArray = Array.isArray(brokerHistory) ? brokerHistory : brokerHistory?.data || [];
  ...
  const topBrokers = useMemo(() => {
    ...
    const validBrokers = brokersOnly.filter(
      (broker) => Number(broker[totalPrice] || 0) > 0 && Number(broker[soldCount] || 0) > 0
    );
    return validBrokers.sort((a, b) => Number(b[totalPrice]) - Number(a[totalPrice])).slice(0, 10);
  }, [brokersOnly, selectedPropertyType]);

  const points = useMemo(() => {
    const { totalPrice, soldCount } = getPropertyFields(selectedPropertyType);
    return topBrokers.map((broker) => ({
      x: broker.agent_name || broker.broker_name || `${broker.first_name || ""} ${broker.last_name || ""}`.trim() || "غير معروف",
  ...
  const options: ApexCharts.ApexOptions = {
    chart: { type: "scatter", toolbar: { show: false }, zoom: { enabled: false }, foreColor: "#6b7280", background: "#fff" },
    xaxis: { type: "category", labels: { rotate: -25, style: { fontSize: "12px", colors: "#374151" } }, axisBorder: { show: true, color: "#00000078" } },
    yaxis: { min: 0, labels: { offsetX: -1 }, axisBorder: { show: true, color: "#00000078" } },
    markers: { size: 8, colors: ["#c5b48a"], strokeColors: "#c5b48a" },
    grid: { borderColor: "#00000078", xaxis: { lines: { show: true } }, yaxis: { lines: { show: true } } },
    tooltip: {
      theme: "light",
    ...
    title: { text: "", align: "right", style: { fontWeight: "700", fontSize: "20px", color: "#000" } },
  };
