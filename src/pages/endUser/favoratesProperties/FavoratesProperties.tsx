...
import useGetFavoriteProperties from "@/hooks/endUser/useGetFavoriteProperties";
import { useRemoveFromFavorites } from "@/hooks/endUser/useRemoveFromFavorites";
...
const FavoriteProperties = () => {
  const { favoriteProperties } = useGetFavoriteProperties();
  const { handleRemoveFromFavorites } = useRemoveFromFavorites();
    ...
    <AnimateContainer>
      <PageContainer>
        <div className="mb-6 pb-4">
          <h1 className="text-2xl md:text-3xl font-bold">العقارات المفضلة</h1>
          <p className=" mt-2 text-xl">
            يتم عرض جميع العقارات النشطة المرسلة اليك من قبل الوكيل العقاري
            والتي حازت على إعجابك
          </p>
          <hr className="mt-2xl" />
        </div>
