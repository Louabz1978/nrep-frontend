...
>(
  (
    { className, children, showTrigger, showSortIcon, ...props },
    forwardedRef
  ) => {
      ...
      <PopoverPrimitive.Trigger ref={forwardedRef as any} asChild>
        <div
          ...
          className={cn(
            ...
            className,
            {
              "max-w-full border-none": !showTrigger,
            }
          )}
...
>(function MultiSelectValue(
  ...
  return (
    <TooltipProvider delayDuration={300}>
      <div
        className={cn(
          "flex w-full flex-1 flex-wrap items-center gap-1.5 overflow-x-hidden overflow-y-hidden",
          className
        )}
        {...props}
        ref={forwardRef}
        ...
        {renderItems.map((value) => {
          ...
          const el = (
            <Badge
              variant="outline"
              key={value}
              className="group/multi-select-badge cursor-pointer rounded-full px-2 py-1 pr-1 leading-4"
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
...
>(function MultiSelectContent({ className, children, ...props }, ref) {
...
>(function MultiSelectItem(
  ...
  return (
    <CommandItem
      value={value}
      className={cn(
        disabled && "cursor-not-allowed text-card-foreground",
        className
      )}
      disabled={disabled}
