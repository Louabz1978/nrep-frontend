...
export default function ColumnSearch({
  ...
  function renderInputNode() {
    switch (type) {
      ...
      //       noResultsMessage={t("general.no_results")}
      //       placeholder={t(label)}
      //     />
      //   );
      // case "date":
      //   return (
      //     <DatePicker value={inputValue as Date} onSelect={handleDateSelect} />
      //   );
      default:
        return null;
    }
  ...
  function renderStateLabel() {
    ...
    if (type === "select") {
      ...
      // return selected ? `: ${t(selected.label)}` : `: ${q}`;
  ...
  return (
    <Popover>
      ...
      <PopoverContent className="w-80 rounded-sm bg-white shadow-sm border z-[1310] ">
