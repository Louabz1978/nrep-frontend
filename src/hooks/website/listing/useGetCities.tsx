...
function useGetCities({ search = "", perPage = 20 } = {}) {
  ...
  const queryParams = getSearchParams(searchParams, `${TABLE_PREFIXES.cities}_`);

  const query = useInfiniteQuery({
    queryKey: [QUERY_KEYS.listings.cities,  JSON.stringify(queryParams) ,search, perPage],
    queryFn: () =>
      getCities({queryParams:queryParams }),
